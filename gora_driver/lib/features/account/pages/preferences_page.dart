...
class _PreferencesPageState extends State<PreferencesPage> {
  bool _taxi = true, _rental = true, _outstation = false, _hireDriver = false, _delivery = false;
  ...
  Future<void> _loadCurrent() async {
    try {
      ...
      setState(() {
        ...
        if (d.containsKey('acceptsDelivery') && d['acceptsDelivery'] is bool) _delivery = d['acceptsDelivery'] as bool;
      });
    } catch (_) {}
  }
  ...
  Future<void> _savePrefs() async {
    ...
    try {
      final res = await DriverApiService.updatePreferences({'acceptsOutstation': _outstation, 'acceptsRental': _rental, 'acceptsHireDriver': _hireDriver, 'acceptsDelivery': _delivery});
      if (!mounted) return;
      if (res['error'] != null) {
      ...
      final savedDel = (prefs?['acceptsDelivery'] as bool?) ?? _delivery;
      setState(() { _outstation = savedOut; _rental = savedRent; _hireDriver = savedHire; _delivery = savedDel; });
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Saved — Outstation:${savedOut ? 'ON' : 'OFF'} Rental:${savedRent ? 'ON' : 'OFF'} Hire:${savedHire ? 'ON' : 'OFF'} Parcel:${savedDel ? 'ON' : 'OFF'}'),
