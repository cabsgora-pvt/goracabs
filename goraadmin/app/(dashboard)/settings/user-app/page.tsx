    ...
    <div className="p-6 max-w-5xl">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={save} disabled={saving} className="bg-[#1C2656] text-white px-5 py-2 rounded-lg text-sm font-semibold disabled:opacity-50">{saving ? 'Saving…' : 'Save All'}</button>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex gap-2 flex-wrap border-b mb-4">
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-3 py-2 text-sm font-semibold rounded-t-lg ${tab === t ? 'bg-[#1C2656] text-white' : 'text-gray-600 hover:bg-gray-100'}`}>{t}</button>
        ))}
      </div>
          ...
          {arr('banners').map((b: any, i: number) => (
            <Card key={i} onDel={() => delRow('banners', i)}>
              ...
              <div className="flex items-center gap-2">
                <input type="color" value={b.color || '#1C2656'} onChange={e => upd('banners', i, 'color', e.target.value)} />
          ...
          {arr('coupons').map((c: any, i: number) => (
            <Card key={i} onDel={() => delRow('coupons', i)}>
              ...
              <div className="grid grid-cols-2 gap-2">
                <select className={inp} value={c.discountType || 'flat'} onChange={e => upd('coupons', i, 'discountType', e.target.value)}>
                ...
                <select className={inp} value={c.service || 'all'} onChange={e => upd('coupons', i, 'service', e.target.value)}>
        ...
        <Section onAdd={() => addRow('whyChooseUs', { icon: 'shield', title: '', desc: '' })}>
          {arr('whyChooseUs').map((w: any, i: number) => (
            <Card key={i} onDel={() => delRow('whyChooseUs', i)}>
              <select className={inp} value={w.icon || 'shield'} onChange={e => upd('whyChooseUs', i, 'icon', e.target.value)}>
      ...
      {tab === 'Support' && (
        <div className="bg-white border rounded-xl p-4 space-y-3 max-w-md">
          <Field label="Phone"><input className={inp} value={cfg.support?.phone || ''} onChange={e => setCfg({ ...cfg, support: { ...cfg.support, phone: e.target.value } })} /></Field>
          <Field label="Email"><input className={inp} value={cfg.support?.email || ''} onChange={e => setCfg({ ...cfg, support: { ...cfg.support, email: e.target.value } })} /></Field>
          <Field label="WhatsApp"><input className={inp} value={cfg.support?.whatsapp || ''} onChange={e => setCfg({ ...cfg, support: { ...cfg.support, whatsapp: e.target.value } })} /></Field>
          <Field label="Address"><input className={inp} value={cfg.support?.address || ''} onChange={e => setCfg({ ...cfg, support: { ...cfg.support, address: e.target.value } })} /></Field>
        </div>
      )}
      ...
      {tab === 'Referral' && (
        <div className="bg-white border rounded-xl p-4 space-y-3 max-w-md">
          <Field label="Reward Amount (₹)"><input className={inp} type="number" value={cfg.referral?.rewardAmount || 0} onChange={e => setCfg({ ...cfg, referral: { ...cfg.referral, rewardAmount: +e.target.value } })} /></Field>
          <Field label="Message"><input className={inp} value={cfg.referral?.message || ''} onChange={e => setCfg({ ...cfg, referral: { ...cfg.referral, message: e.target.value } })} /></Field>
        </div>
      )}
    </div>
  )
...
function Section({ children, onAdd }: { children: React.ReactNode; onAdd: () => void }) {
  return (
    <div className="space-y-3">
      {children}
      <button onClick={onAdd} className="text-sm font-semibold text-[#1C2656] border border-dashed border-[#1C2656] rounded-lg px-4 py-2 w-full hover:bg-[#1C2656]/5">+ Add</button>
    </div>
  )
}
function Card({ children, onDel }: { children: React.ReactNode; onDel: () => void }) {
  return (
    <div className="bg-white border rounded-xl p-3 space-y-2 relative">
      <button onClick={onDel} className="absolute top-2 right-2 text-red-500 text-xs font-bold">✕ Remove</button>
      {children}
    </div>
  )
}
