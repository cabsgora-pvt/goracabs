...
export default function OutstationServicePage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault()
    setSaving(true)
    const res = await fetch('/api/services/outstation', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...config, service: 'outstation' }),
    })
    setSaving(false)
    setToast(res.ok ? { msg: 'Outstation pricing saved!', type: 'success' } : { msg: 'Failed to save', type: 'error' })
  }

  if (loading) return (
    <div><Header title="Outstation Pricing" />
  ...
  return (
    <div>
      <Header title="Outstation Pricing" />
